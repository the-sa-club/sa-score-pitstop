...
import {ErrorModalTypes, InfoModalTypes, InvoiceResources, WaitingSignature} from "../data/types";
...
export const Resources: React.FC<Props> = () => {
    ...
    const [customBuySupply, setCustomBuySupply] = React.useState<{ open: boolean }>({open: false});
        ...
        if (inventory && (selectedFleets.length > 0 || allFleets.length > 0)) {
            try {
                if (publicKey && signTransaction && signAllTransactions) {
                    ...
                    const waitingSigntaures = signatures.map<WaitingSignature>(signature => ({
                        hash: signature,
                        status: 'processing'
                    }));
    ...
    return (
        <>
            ...
            {customBuySupply.open ?
                <CustomSupplyModal
                    data={{
                        inventory: inventory!, resourcesData
                    }}
