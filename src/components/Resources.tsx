...
import { BUY_SUPPLY_MODES, CONN, PALETTE } from "../constants";
...
export const Resources: React.FC<Props> = () => {
  ...
  const [customBuySupply, setCustomBuySupply] = React.useState<{
    open: boolean;
  }>({ open: false });
  ...
  const onFullTankBuyClick = async () => {
    setCustomBuySupply({ open: true });
  };
  ...
  const onBuySupplyClick = async () => {
    if (
      publicKey &&
      inventory &&
      (selectedFleets.length > 0 || allFleets.length > 0)
    ) {
      //! Full Tanks click  dont forget 20
      // if (buySupplyMode == BUY_SUPPLY_MODES.CUSTOM) {
      //     return onFullTankBuyClick();
      // }
      await createInvoice();
    }
  };
  ...
  const onSettleFundsClick = async () => {
    try {
      if (publicKey && signTransaction) {
        ...
        setLoadingInfo({
          loading: true,
          message: "Preparing Settle Transactions",
          pct: 40,
        });
        ...
        FleetService.checkSignatures([
          { hash: signature, status: "processing" },
        ]);
    ...
    if (inventory && (selectedFleets.length > 0 || allFleets.length > 0)) {
      try {
        if (publicKey && signTransaction && signAllTransactions) {
          setLoadingInfo({
            loading: true,
            message: "Preparing Resupply Transactions",
            pct: 20,
          });
  ...
  const createInvoice = async (resourcesToBuy?: InvoiceResources) => {
    if (
      publicKey &&
      inventory &&
      (selectedFleets.length > 0 || allFleets.length > 0)
  ...
  return (
    <>
        ...
        //   onClick={onSettleFundsClick} 
          ...
          <SupplyButton
            onClick={() => {
              ...
              //     return;
              // }
              return onResupplyClick();
            }}
            // disabled={buySupplyMode == BUY_SUPPLY_MODES.CUSTOM}
          >
      ...
      {customBuySupply.open ? (
        <CustomSupplyModal
          data={{
            inventory: inventory!,
            resourcesData,
          }}
          open={customBuySupply.open}
          onClose={(resourcesToBuy?: InvoiceResources) => {
            setCustomBuySupply({
              open: false,
            });
            if (resourcesToBuy) {
              return createInvoice(resourcesToBuy);
            }
          }}
        />
      ) : (
